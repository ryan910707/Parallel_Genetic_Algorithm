...
int main(int argc, char** argv) {
    ...
    while (generation < GENERATIONS) {
        ...
        // #pragma acc parallel loop copyin(new_population[0:POP_SIZE]) copy(population[0:POP_SIZE])
...
void evaluate_population(Individual population[]) {
    ...
    // #pragma acc parallel loop copy(population[0:POP_SIZE])
    for (int i = 0; i < POP_SIZE; i++) {
        int total_value = 0;
        int total_weight = 0;
        // #pragma acc parallel loop reduction(+:total_value)
        for (int i = 0; i < ITEMS_NUM; i++) {
        ...
        // #pragma acc parallel loop reduction(+:total_weight)
        for (int i = 0; i < ITEMS_NUM; i++) {
            if (individual.genes[i] == 1) {
...
void selection(Individual population[], Individual new_population[]) {
    int q = rand();
    int p = rand();
    #pragma acc parallel loop copyin(population[0:POP_SIZE], q, p) copy(new_population[0:POP_SIZE])
    for (int i = 0; i < POP_SIZE; i++) {
        // Tournament selection: pick two random individuals and select the best one
        int parent1 = q % POP_SIZE;
        int parent2 = p % POP_SIZE;
